...
#' @param SL.ftime A character vector or list specification to be passed to the \code{SL.library} option 
#' in the call to \code{SuperLearner} for the cause-specific hazards. See \code{?SuperLearner} for more information on how to specify valid 
...
#' @param bounds A \code{data.frame} of bounds on the conditional hazard function. The \code{data.frame} should have a column
...
#' @param verbose A boolean indicating whether the function should print messages to indicate progress. If \code{SuperLearner}
#' is called internally, this option will additionally be passed to \code{SuperLearner}.
#' @param tol The stopping criteria. The TMLE algorithm performs
...
hazard_tmle <- function(
  ...
  out <- list(est=est,var=var, meanIC=meanIC, ic=infCurves, trtMod=trtMod, ftimeMod=ftimeMod,
              ctimeMod=ctimeMod, ftime = ftime, ftype = ftype, trt = trt, adjustVars = adjustVars)
  class(out) <- "survtmle"
  return(out) 
}
