  ...
  for (z in uniqtrt) {
    for (j in ofInterestJ) {
      dat[[paste0("margF", j, ".z", z, ".t0")]] <-
        mean((dataList[[1]]$weights * dataList[[1]][[paste0("F", j, ".z", z, ".t0")]])[dataList[[1]]$t == min(dataList[[1]]$t)])

      dat[[paste0("F", j, ".z", z, ".t0")]] <-
        (dataList[[1]]$weights * dataList[[1]][[paste0("F", j, ".z", z, ".t0")]])[dataList[[1]]$t == min(dataList[[1]]$t)]
      thisD <- NULL
      for (jTild in allJ) {
        H <- paste0("H", j, ".j", ifelse(jTild == j, "Self", "NotSelf"), ".z", z)
        thisD <- cbind(thisD, dataList[[1]]$weights * dataList[[1]][[H]] / (1 - dataList[[1]][[paste0("hazNot", j)]]) *
