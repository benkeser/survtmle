...
estimateTreatment <- function(dat,
  ...
  if (length(unique(dat$trt)) == 1) {
    ...
    a_ct <- 0
    gn_A <- vector(mode = "list", length = ntrt)
    fm_A <- vector(mode = "list", length = ntrt - 1)
    for (a in uniqtrt_sort[-length(uniqtrt)]) {
      # determine who to include in the regression for this outcome
      if (a_ct == 0) {
        include <- rep(TRUE, length(dat$trt))
      } else {
        include <- !(dat$trt %in% uniqtrt_sort[1:a_ct])
      }
      if (!is.null(SL.trt)) {
      ...
      fm_A[[a_ct + 1]] <- tmp_fm
      names(fm_A)[a_ct + 1] <- paste0("I(trt = ", a, ") ~ adjustVars")
      a_ct <- a_ct + 1
    }
    # add in final predictions
    gn_A[[a_ct + 1]] <- 1 - Reduce("+", gn_A[1:a_ct])
    dat[[paste0("g_", uniqtrt_sort[ntrt])]] <- gn_A[[a_ct + 1]]
  }
