...
checkInputs <- function(ftime, 
  ...
  Gcomp = FALSE){
	...
	if(any(ftime<=0)){
		stop("Some failure times less than or equal zero. Remove these observations and try again")
	}
	...
	for(j in ftypeOfInterest){
		for(z in trtOfInterest){
			if(sum(ftype==j & trt == z) == 0){
				stop(paste0("No endpoints of type ftype=",j," observed in trt=",z," group. Adjust ftypeOfInterest and trtOfInterest accordingly."))
			}
		}
	}
	...
	if(any(ftype < 0)){
		stop("Some ftype less than 0. ftype should use 0 to denote censoring and positive numeric values to denote other endpoints.")
	}
			...
			for(z in trtOfInterest){
				if(t0 > max(ftime[ftype == j & trt == z])){
					warning(paste0("t0 larger than last observed endpoint of ftype=",j,
					               " and trt=",z,". Hazard TMLE may be extrapolating to estimate incidence."))
				}
			}
		}
	}
	...
	if(t0 > max(ftime[ftype > 0]) & method == "mean"){
		warning("t0 larger than last observed endpoint. Mean-based TMLE assumes constant incidence between last observed failure type and this time.")
	}
